...
class RBM(object):
    ...
    def training(self, train, valid, user, epochs, batchsize, free_energy, verbose, filename):
        ...
        vb = tf.placeholder(tf.float32, [self.num_vis])
        # Number of features were going to learn
        hb = tf.placeholder(tf.float32, [self.num_hid])
        W = tf.placeholder(
            tf.float32, [self.num_vis, self.num_hid])  # Weight Matrix
        ...
        for i in range(epochs):
            ...
            if valid:
                etrain = np.mean(util.free_energy(
                    train, cur_w, cur_vb, cur_hb))
                self.energy_train.append(etrain)
                evalid = np.mean(util.free_energy(
                    valid, cur_w, cur_vb, cur_hb))
...
#         cur_user_id = ratings[ratings['user_id']
        ...
        unvisited = attractions[~attractions['attraction_id'].isin(
            places_id)]['attraction_id']
        unvisited_id = unvisited.tolist()

        # extract the ratings of all the unread books from ratings dataframe
        unseen_with_score = ratings[ratings['attraction_id'].isin(
            unvisited_id)]

        # grouping the unread data on book id and taking the mean of the recommendation scores for each book_id
        grouped_unseen = unseen_with_score.groupby('attraction_id', as_index=False)[
...
class Util(object):
    ...
    def read_data(self, folder):
        ...
        print(os.path.isfile(os.path.join(folder, "attraction_reviews.json")))
...
def get_recc(att_df, cat_rating):
    ...
    dirr = folder + '\ml_scripts\etl'
    t = os.path.join(dirr, 'attraction_reviews.json')
    print(t, os.path.isfile(t))
    ratings, attractions = util.read_data(dirr)
    ...
    joined = ratings.set_index('attraction_id').join(attractions[[
        "attraction_id", "category"]].set_index("attraction_id")).reset_index('attraction_id')
    ...
    cat_rat_df = category_df.set_index(
        'user_id').join(rating_df.set_index('user_id'))
    ...
    filename = "e"+str(epochs)+"_r"+str(rows)+"_lr" + \
        str(alpha)+"_hu"+str(H)+"_bs"+str(batch_size)
...
def filter_df(filename, user, low, high, province, att_df):
    recc_df = pd.read_csv('recommendations/'+filename +
                          '/user{u}_unseen.csv'.format(u=user), index_col=0)
    recc_df.columns = ['attraction_id', 'att_name',
                       'att_cat', 'att_price', 'score']
    recommendation = att_df[['attraction_id', 'name', 'category', 'city', 'latitude', 'longitude', 'price', 'province', 'rating']].set_index(
        'attraction_id').join(recc_df[['attraction_id', 'score']].set_index('attraction_id'), how="inner").reset_index().sort_values("score", ascending=False)

    filtered = recommendation[(recommendation.province == province) & (
        recommendation.price >= low) & (recommendation.price >= low)]
    folder = os.path.dirname(__file__)
    folder = folder + "\ml_scripts\outputs"
    fil = os.path.join(folder, 'attractions_cat.json')
    print(fil, os.path.isfile(fil))
    url = pd.read_json(fil, orient='records')
    url['id'] = url.index
    with_url = filtered.set_index('attraction_id').join(
        url[['id', 'attraction']].set_index('id'), how="inner")
    print(with_url.head())
    return with_url
    ...
    try:
        while(1):
            ...
            if(first_recc['name'].values.T[0] not in final['name']):
                ...
                final['location'].append(
                    first_recc[['latitude', 'longitude']].values.tolist()[0])
                final['price'].append(first_recc['price'].values.T[0])
                final['rating'].append(first_recc['rating'].values.T[0])
                final['image'].append(
                    get_image(first_recc['name'].values.T[0]))
...
def find_closest(with_url, loc, tod, final):
    ...
    sorted_d = with_dist.sort_values(
        ['distance', 'price'], ascending=['True', 'False'])
    if tod == "Evening":
...
def final_output(days, final):
    ...
    for i in range(days):
        ...
        tab['name'].append([re.sub('_', ' ', i).capitalize()
                           for i in final['name'][i*4:(i+1)*4]])
        tab['category'].append([re.sub('_', ' ', i).capitalize()
                               for i in final['category'][i*4:(i+1)*4]])
        tab['location'].append(
            ["("+str(i[0])+","+str(i[1])+")" for i in final['location'][i*4:(i+1)*4]])
...
def feed_input(price_low, price_high, province, days, cat_rating):
    #dirr = PurePosixPath('attractions.json')
    #dirr = '/home/arppit/Documents/Varun/tripsero/tripsero/etl/attractions.json'
    print('*'*100)
    folder = os.path.dirname(__file__)
    dirr = folder + '/ml_scripts/etl/attractions.json'
    att_df = pd.read_json(dirr, orient='records')

    category_df = att_df.groupby('category').size(
    ).reset_index().sort_values([0], ascending=False)[:20]
    ...
    with_url = filter_df(filename, user, price_low,
                         price_high, province, att_df)
    ...
    for i in range(len(final['timeofday'])):
        if i % 4 == 0:
            ...
            final = find_closest(
                with_url, final['location'][-1], final['timeofday'][i], final)

    # print(final)
    t = final_output(days, final)
    return t
    # return json.dumps(t)
#g = feed_input(low,high,province,3)
# print(g)
